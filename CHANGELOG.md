...
- **Historical Session Restoration Resilience**:
  ...
  - Added compile-time `CARGO_MANIFEST_DIR` and `ZED_REPO_ROOT` fallback to `dev_repo_root()` in `util.rs` (Patch 0035), preventing asset loading panics when launched outside the checkout.
  - Preserved standard macOS Application Support data directory and restored all 92 sidebar threads while respecting `KATALYST_STATE_HOME` overrides (Patch 0036).
## [0.2.2] - 2026-09-14
